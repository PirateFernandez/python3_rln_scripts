...
"""These two lines remove the ticks and their labels."""
ax.set_xticklabels([])
ax.set_yticklabels([])
"""These two lines draw curved lines at x -120, -60, 60, 120."""
ax.plot(x_60_rad, np.arange(-1.5, 2, 0.5), color='silver', lw=1.5, linestyle=':')
ax.plot(x_m60_rad, np.arange(-1.5, 2, 0.5), color='silver', lw=1.5, linestyle=':')
ax.plot(x_120_rad, np.arange(-1.5, 2, 0.5), color='silver', lw=1.5, linestyle=':')
ax.plot(x_m120_rad, np.arange(-1.5, 2, 0.5), color='silver', lw=1.5, linestyle=':')
"""These two lines draw vertical and horizontal straight lines as x, y cartesian axes."""
ax.vlines(0,-1.6,1.6, colors='silver', lw=1.5, linestyles=':')
ax.hlines(0,-10,10, colors='silver', lw=1.5, linestyles=':')
"""Write a png dpi 300 file at the same location where the star file is."""
plt.savefig(f"{root_path}", dpi=300)
plt.show()
